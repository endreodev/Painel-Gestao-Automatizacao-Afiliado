...
Tudo que o sistema faz e por quê. Para só colocar no ar, comece pelo
[README](README.md) — são quatro comandos.
...
Roda o sistema inteiro num container só: **PHP** (o sistema), **Node** (a ponte do
WhatsApp) e **Chromium** (a coleta) na mesma imagem. Estão juntos por necessidade:
o PHP inicia os outros dois como processos filhos e fala com eles por `127.0.0.1`.
Em containers separados a ponte subiria duas vezes na mesma sessão do WhatsApp, que
é justamente o que derruba o pareamento.

```bash
docker compose build              # monta a imagem (uns 5 min na primeira vez)
docker compose run --rm importar  # traz seu storage/ atual para dentro do volume
docker compose up -d              # sobe painel + laço de publicação
```

Painel em **http://127.0.0.1:8321**. Se o WhatsApp pedir QR:
...
| O que | Comando |
|---|---|
| Situação | `docker compose exec mlgroup php bin/mlgroup status` |
| Ler o QR | `docker compose exec mlgroup php bin/mlgroup conectar` |
| Trocar de grupo | `docker compose exec mlgroup php bin/mlgroup grupos` |
| Um ciclo agora | `docker compose exec mlgroup php bin/mlgroup ciclo` |
| Prévia da mensagem | `docker compose exec mlgroup php bin/mlgroup previa` |
| Acompanhar | `docker compose logs -f mlgroup` |
| Parar | `docker compose stop` |

O container roda `painel` e `rodar` juntos (`docker/entrada.sh`). Para subir só um
deles: `docker compose run --rm --service-ports mlgroup painel`.

### O que muda em relação a rodar no Windows

**`storage/` fica num volume do Docker, não na pasta do projeto.** O banco roda em
modo WAL, que depende de trava e memória compartilhada de verdade — a ponte de
arquivos entre Windows e Linux não entrega isso, e o banco trava ou devolve
*disk I/O error* no meio de um ciclo. Por isso o `importar`: ele copia o seu
`storage/` (banco, sessão do WhatsApp, `config-local.json`, histórico) para dentro
do volume, uma vez. **Pare o sistema que roda no Windows antes de importar** — copiar
o banco no meio de uma gravação traz um arquivo pela metade.

Para tirar uma cópia de volta:

```bash
docker compose stop
docker cp mlgroup:/app/storage ./storage-do-container
```

**`.env`, `config/` e `templates/` continuam sendo seus arquivos**, montados de fora
e editáveis do jeito de sempre. Alterar `config/*.php` vale no ciclo seguinte, sem
rebuild. Mexer em `src/` exige `docker compose up -d --build`.

**Não existe tarefa agendada.** `php bin/mlgroup agendar` é do Windows; aqui quem
mantém tudo de pé é o `restart: unless-stopped` do compose, e o próprio laço já
respeita a janela de horário da `agenda`.

**`ml-login` não roda no container.** A tela do Link Builder do Mercado Livre exige
login com reCAPTCHA — precisa de gente digitando numa janela de verdade, e não há
tela dentro do container. O sistema não para por causa disso: sem link oficial ele
cai para `afiliado.modo = 'modelo'`, que leva a sua tag do mesmo jeito. Quem quiser
o link oficial faz o login rodando o sistema direto no Windows.

### Ajustes do compose

| Onde | Por quê |
|---|---|
| `ports: 127.0.0.1:8321:8321` | o painel edita configuração e dispara envio, e não pede senha — a porta não aparece na rede local |
| `PAINEL_HOST=0.0.0.0` | dentro do container o painel precisa escutar em todas as interfaces para o mapeamento de porta alcançá-lo |
| `PAINEL_ORIGENS=...` | a requisição chega pelo gateway do Docker (um `172.x`), e não mais de `127.0.0.1`; sem isso o dono da máquina levaria 403 |
| `MLG_CHROME_ARGS=--no-sandbox ...` | o Chromium se recusa a subir como root com o sandbox ligado, e o sandbox depende de chamadas que o Docker bloqueia |
| `shm_size: 1gb` | os 64 MB padrão fazem a aba morrer no meio da renderização de listas grandes |
| `stop_grace_period: 60s` | dá tempo do laço terminar o ciclo em andamento antes de o container cair |

### Quando algo não sobe

```bash
docker compose logs mlgroup              # o que o laço e o painel disseram
docker compose exec mlgroup php bin/mlgroup status
docker compose exec mlgroup cat storage/logs/ponte.log
docker compose exec mlgroup chromium --version
```

Painel respondendo **403**: sua rede do Docker está fora das faixas de
`PAINEL_ORIGENS`. Veja qual IP chegou com `docker compose logs mlgroup` e acrescente
a faixa no `docker-compose.yml`.
